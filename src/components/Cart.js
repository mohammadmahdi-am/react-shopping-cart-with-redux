...
function Cart(props) {
    ...
    return (
        <div>
            ...
            {showProceed && (
                <button className="btn btn-danger w-100" onClick={()=>setShowForm(!showForm)}>Proceed -{showPrice()} Toman </button>
            )}
            {(showForm && showProceed) && (
                <div>
                    <form action="" className="d-flex flex-column text-left" onSubmit={(e)=>handleForm(e)}>
                    ...
                    <button className="btn btn-success">Checkout - {showPrice()} Toman</button> 
                    </form>
                </div>
            )}

            {showModal && (
                <div style={{width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.5)",position:"fixed",top:"0",left:"0",display:"flex",justifyContent:"center",alignItems:"center"}}>
                    <div style={{width:"50vw",backgroundColor:"rgba(255,255,255,1)",position:"absolute"}}>
                        <button style={{position:"absolute",top:"10px",left:"10px"}}className="btn btn-danger" onClick={()=>{setShowModal(false);props.setCartItems([]);}}>X</button>
